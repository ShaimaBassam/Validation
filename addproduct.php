...
$select_products = $db->prepare("SELECT * FROM `products` WHERE product_name = ?");
$select_products->execute([$name]);

if($select_products->rowCount() > 0){
    $message[] = 'product name already exist!';
}else{
    $insert_products = $db->prepare("INSERT INTO `products`(product_name,price,product_desc,image,category_id ) VALUES(?,?,?,?,?)");
    $insert_products->execute([$name, $price, $details, $image, $category_name]);
